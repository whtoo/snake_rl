...
class DQNAgent:
    ...
    def _prepare_batch_for_update(self):
        ...
        if isinstance(self.memory, PrioritizedReplayBuffer):
            states, actions, rewards, next_states, dones, tree_indices, weights_tensor = self.memory.sample(
                self.batch_size)
            ...
            states, actions, rewards, next_states, dones = self.memory.sample(
                self.batch_size)
            update_indices = None
            # Ensure weights is defined and on device
    ...
    def update_model(self):  # Signature reverted
        ...
        with torch.no_grad():
            ...
            target_q_values = rewards + \
                (1 - dones) * self.gamma * next_q_values_target

        td_errors = torch.abs(q_values - target_q_values)
        # td_errors needs to be [batch_size, 1] like weights
    ...
    def load_model(self, path):
        ...
        try:
            ...
            if "Missing key(s)" in str(e) or "Unexpected key(s)" in str(e):
                ...
                self.target_model.load_state_dict(
                    checkpoint['target_model_state_dict'], strict=False)
            else:
                raise e
...
class RainbowAgent(DQNAgent):
    ...
    def __init__(self, model, target_model, env, device,
                 # Renamed n_step to base_n_step, added max_n_step
                 base_n_step=3, max_n_step=10,
        ...
        # Retained for adapt_n_step_freq logic
        self._rainbow_training_updates_count = 0
    ...
    def store_experience(self, state, action, reward, next_state, done):
        ...
        if self.augmenter:
            ...
            aug_s, _, _, aug_ns, _ = self.augmenter.augment(
        ...
        n_step_exp = self.n_step_buffer.add(
            current_state, current_action, current_reward, current_next_state, current_done)
        if n_step_exp is not None:
            self.memory.push(n_step_exp.state, n_step_exp.action,
                             n_step_exp.reward, n_step_exp.next_state, n_step_exp.done)
        if done:
            remaining_exps = self.n_step_buffer.get_last_n_step()
            for exp in remaining_exps:
                self.memory.push(exp.state, exp.action,
                                 exp.reward, exp.next_state, exp.done)
    ...
    def update_model(self):  # Signature reverted
        ...
        if self.use_distributional:
            loss = self._compute_distributional_loss(
        ...
        td_errors_numpy = self._calculate_n_step_td_errors(
        ...
        # Use the same counter for n-step adaptation frequency
        if self._rainbow_training_updates_count > 0 and self._rainbow_training_updates_count % self.adapt_n_step_freq == 0:
    ...
    def _calculate_n_step_td_errors(self, states, actions, rewards, next_states, dones):
        ...
        with torch.no_grad():  # All calculations for TD error should not affect gradients
            if self.use_distributional:
                ...
                current_q_selected_dist_probs = current_dist_probs_model.gather(
                    1, actions.unsqueeze(2).expand(-1, -1, self.n_atoms)).squeeze(1)
                current_q_selected_expected = (
                    current_q_selected_dist_probs * self.support.unsqueeze(0)).sum(1, keepdim=True)

                # Re-calculate target Qs expected value (Double DQN style for action selection)
                next_dist_probs_model = self.model(next_states)
                next_q_values_model = (
                    next_dist_probs_model * self.support.unsqueeze(0)).sum(2)
                next_actions = next_q_values_model.max(1)[1].unsqueeze(1)

                next_dist_probs_target_net = self.target_model(next_states)
                next_best_dist_target_net = next_dist_probs_target_net.gather(
                    1, next_actions.unsqueeze(2).expand(-1, -1, self.n_atoms)).squeeze(1)
                next_q_max_expected = (
                    next_best_dist_target_net * self.support.unsqueeze(0)).sum(1, keepdim=True)

                target_q_for_td_error = rewards + \
                    (1 - dones) * (self.gamma **
                                   self.n_step_buffer.current_n_step) * next_q_max_expected
                td_errors_tensor = torch.abs(
                    current_q_selected_expected - target_q_for_td_error)
            else:  # Standard DQN
                current_q_model_vals = self.model(states).gather(1, actions)

                next_q_values_model = self.model(next_states)
                next_actions = next_q_values_model.max(1)[1].unsqueeze(1)
                next_q_target_net = self.target_model(
                    next_states).gather(1, next_actions)

                target_q_for_td_error = rewards + \
                    (1 - dones) * (self.gamma **
                                   self.n_step_buffer.current_n_step) * next_q_target_net
                td_errors_tensor = torch.abs(
                    current_q_model_vals - target_q_for_td_error)
    ...
    def _compute_standard_loss(self, states, actions, rewards, next_states, dones, weights):
        ...
        with torch.no_grad():
            ...
            target_q_values = rewards + \
                (1 - dones) * (self.gamma **
                               self.n_step_buffer.current_n_step) * next_q_values_target
        # td_errors = torch.abs(q_values - target_q_values) # Ensure td_errors has shape [batch_size, 1]
        # Loss is (q_values - target_q_values).pow(2) * weights or huber_loss
        ...
        # This is MAE like, but original was also this for PER error calc
        loss = (torch.abs(td_errors) * weights).mean()
        return loss
    ...
    def _compute_distributional_loss(self, states, actions, rewards, next_states, dones, weights):
        ...
        current_probs_selected = current_dist_probs.gather(
            1, actions.unsqueeze(2).expand(-1, -1, self.n_atoms)).squeeze(1)
        ...
        with torch.no_grad():
            ...
            next_dist_probs_target_net = self.target_model(next_states)
            next_dist_target = next_dist_probs_target_net.gather(1, next_actions.unsqueeze(
                1).unsqueeze(2).expand(-1, -1, self.n_atoms)).squeeze(1)
            ...
            # Use current_n_step from adaptive buffer for discounting future rewards
            target_support = rewards_exp + \
                (1 - dones_exp) * (self.gamma **
                                   self.n_step_buffer.current_n_step) * support_exp
            ...
            target_dist = torch.zeros_like(
                next_dist_target, device=self.device)

            m_l_contrib = next_dist_target * (upper_indices.float() - b)
            m_u_contrib = next_dist_target * (b - lower_indices.float())

            m_l_contrib_ne = torch.where(
                ne_mask, m_l_contrib, torch.zeros_like(m_l_contrib))
            m_u_contrib_ne = torch.where(
                ne_mask, m_u_contrib, torch.zeros_like(m_u_contrib))

            target_dist.scatter_add_(1, lower_indices.clamp(
                0, self.n_atoms - 1), m_l_contrib_ne)
            target_dist.scatter_add_(1, upper_indices.clamp(
                0, self.n_atoms - 1), m_u_contrib_ne)

            if eq_mask.any():
                src_eq = torch.where(
                    eq_mask, next_dist_target, torch.zeros_like(next_dist_target))
                target_dist.scatter_add_(
                    1, lower_indices.clamp(0, self.n_atoms - 1), src_eq)
